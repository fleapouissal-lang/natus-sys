-- Corrige purge_operational_data : WHERE obligatoire + stock initial 100 par magasin/dépôt.
...
BEGIN
  ...
  SELECT s.id, p.id, v_default_stock
  FROM stores s
  CROSS JOIN products p
  WHERE s.is_active = true
  ON CONFLICT (store_id, product_id) DO UPDATE
  SET stock = v_default_stock, updated_at = NOW();
