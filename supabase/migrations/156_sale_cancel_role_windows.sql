-- Annulation vente : caissier 2 h, gérant 8 h (même jour calendaire), directeur sans limite.
...
BEGIN
  ...
  IF v_role = 'cashier' THEN
    ...
    ELSIF v_sale.cashier_id <> auth.uid() THEN
      RAISE EXCEPTION 'Vous ne pouvez annuler que vos propres ventes';
    END IF;
    IF v_sale.created_at < now() - interval '2 hours' THEN
      RAISE EXCEPTION 'Annulation impossible après 2 h — contactez le gérant';
    END IF;
  ELSIF v_role IN ('manager', 'directeur', 'admin') THEN
    IF v_role = 'manager' THEN
      ...
      v_sale_calendar_date := (timezone('Africa/Casablanca', v_sale.created_at))::date;
      IF v_sale_calendar_date <> store_calendar_date() THEN
        RAISE EXCEPTION 'Annulation impossible : la vente n''est pas du jour — contactez le directeur';
      END IF;

      IF v_sale.created_at < now() - interval '8 hours' THEN
        RAISE EXCEPTION 'Annulation impossible après 8 h — contactez le directeur';
      END IF;
    END IF;
  ELSE
