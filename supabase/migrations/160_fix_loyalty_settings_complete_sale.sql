-- Fix : « null value in column "loyalty_points_earned" of relation "sales" »
--
-- Cause : la purge des données opérationnelles (migration 150) supprime la
-- ligne unique (id = 1) de loyalty_settings. Dans complete_sale, le
-- « SELECT ... INTO » ne trouve alors aucune ligne et met v_points_per_mad à
-- NULL → FLOOR(v_total / NULL) = NULL → loyalty_points_earned NULL → la vente
-- échoue (contrainte NOT NULL) dès qu'un client est associé (fidélité ou
-- client Pro non encore activé).

-- 1) Réinsère la configuration fidélité si elle a disparu.
INSERT INTO loyalty_settings (id)
VALUES (1)
ON CONFLICT (id) DO NOTHING;

-- 2) update_loyalty_settings devient un UPSERT (robuste après purge).
...
BEGIN
  ...
  INSERT INTO loyalty_settings (id, points_per_mad, point_value_mad, min_points_to_redeem, updated_at, updated_by)
  VALUES (1, p_points_per_mad, p_point_value_mad, p_min_points_to_redeem, NOW(), auth.uid())
  ON CONFLICT (id) DO UPDATE
    SET points_per_mad = p_points_per_mad,
...
-- 3) complete_sale : blindage des paramètres fidélité (valeurs par défaut si
--    la config a été purgée) et garantie que loyalty_points_earned n'est
--    jamais NULL.
...
BEGIN
  ...
  -- Robustesse : si la config fidélité a été purgée, utiliser les défauts.
  v_points_per_mad := COALESCE(NULLIF(v_points_per_mad, 0), 10);
  v_point_value_mad := COALESCE(NULLIF(v_point_value_mad, 0), 1);
  v_min_redeem_points := COALESCE(v_min_redeem_points, 200);
  ...
  IF p_customer_id IS NOT NULL AND NOT v_is_active_pro_client THEN
    v_points_earn := COALESCE(FLOOR(v_total / v_points_per_mad)::INTEGER, 0);
  END IF;

  -- Garde-fou final : jamais NULL.
  v_points_earn := COALESCE(v_points_earn, 0);
  v_points_redeem := COALESCE(v_points_redeem, 0);
