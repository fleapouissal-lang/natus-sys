-- Retours stock dépôt : création par compte hub, validation directeur uniquement
...
AS $$
  SELECT CASE
    ...
    WHEN is_hub_operator() AND EXISTS (
      SELECT 1 FROM stores s
...
BEGIN
  ...
  ELSIF v_role = 'hub' THEN
    ...
    IF v_store_id IS NULL THEN
      RAISE EXCEPTION 'Aucun entrepôt dépôt configuré pour votre ville';
    END IF;
  ELSE
    RAISE EXCEPTION 'Seuls les caissiers et comptes dépôt peuvent créer un retour stock';
  END IF;
...
BEGIN
  ...
  IF COALESCE(v_is_hub, false) THEN
    IF v_role NOT IN ('directeur', 'admin') THEN
      RAISE EXCEPTION 'Seul un directeur peut valider un retour dépôt';
    END IF;
...
BEGIN
  ...
  IF COALESCE(v_is_hub, false) THEN
    IF v_role NOT IN ('directeur', 'admin') THEN
      RAISE EXCEPTION 'Seul un directeur peut refuser un retour dépôt';
    END IF;
  ELSIF v_role NOT IN ('directeur', 'admin', 'manager') THEN
