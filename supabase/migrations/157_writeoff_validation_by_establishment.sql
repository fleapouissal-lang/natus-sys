-- Annulations de stock : magasin (gérant/directeur) vs dépôt Hub (directeur seul)
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Annulation introuvable';
  END IF;
  ...
  IF COALESCE(v_is_hub, false) THEN
    IF v_role NOT IN ('directeur', 'admin') THEN
      RAISE EXCEPTION 'Seul un directeur peut valider une annulation de stock en dépôt (Hub)';
    END IF;
  ELSIF v_role NOT IN ('directeur', 'admin', 'manager') THEN
    RAISE EXCEPTION 'Seuls le gérant ou le directeur peuvent valider une annulation de stock en magasin';
  END IF;

  IF v_writeoff.status <> 'pending' THEN
    RAISE EXCEPTION 'Cette annulation a déjà été traitée';
  END IF;
  ...
  LOOP
    ...
    VALUES (
      ...
      -v_item.quantity,
      'adjustment',
      'Annulation ' || v_item.reason::TEXT || ' — validation ' || p_writeoff_id,
      auth.uid(),
      v_writeoff.store_id
    );
  END LOOP;
...
BEGIN
  ...
  IF COALESCE(v_is_hub, false) THEN
    IF v_role NOT IN ('directeur', 'admin') THEN
      RAISE EXCEPTION 'Seul un directeur peut refuser une annulation de stock en dépôt (Hub)';
    END IF;
  ELSIF v_role NOT IN ('directeur', 'admin', 'manager') THEN
    RAISE EXCEPTION 'Seuls le gérant ou le directeur peuvent refuser une annulation de stock en magasin';
  END IF;
