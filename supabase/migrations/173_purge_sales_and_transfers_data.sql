...
CREATE OR REPLACE FUNCTION purge_sales_and_transfers_data()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_sales INTEGER := 0;
  v_stock_movements_sale INTEGER := 0;
  v_closures INTEGER := 0;
BEGIN
  IF auth.uid() IS NOT NULL AND NOT is_director_or_admin() THEN
    RAISE EXCEPTION 'Seul un directeur ou admin peut purger les ventes et transferts';
  END IF;
  ...
  GET DIAGNOSTICS v_stock_movements_sale = ROW_COUNT;
  ...
  RETURN jsonb_build_object(
    ...
    'deleted', jsonb_build_object(
      ...
      'sales', v_sales,
      'stock_movements_sale', v_stock_movements_sale,
...
REVOKE ALL ON FUNCTION purge_sales_and_transfers_data() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION purge_sales_and_transfers_data() TO service_role;
