-- Facture : email, ICE client + mise à jour avant validation.

ALTER TABLE sales
  ADD COLUMN IF NOT EXISTS customer_ice TEXT;

DROP FUNCTION IF EXISTS update_sale_invoice_customer(UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS update_sale_invoice_customer(UUID, TEXT, TEXT, TEXT, TEXT);
...
BEGIN
  ...
  SET
    ...
    customer_ice = NULLIF(trim(p_customer_ice), '')
  WHERE id = p_sale_id
...
GRANT EXECUTE ON FUNCTION update_sale_invoice_customer(UUID, TEXT, TEXT, TEXT, TEXT) TO authenticated;
