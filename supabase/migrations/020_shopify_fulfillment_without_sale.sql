-- Préparation commande Shopify : sortie stock sans vente caisse
-- Encaissement COD : vente caisse sans re-sortie stock

ALTER TABLE shopify_orders
  ADD COLUMN IF NOT EXISTS fulfilled_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS fulfilled_by UUID REFERENCES profiles(id);

CREATE INDEX IF NOT EXISTS idx_shopify_orders_fulfilled ON shopify_orders(fulfilled_at);

CREATE OR REPLACE FUNCTION fulfill_shopify_order(
  p_items JSONB,
...
BEGIN
  ...
  LOOP
    ...
    VALUES (v_product.id, -v_qty, 'sale', 'Préparation commande Shopify', auth.uid(), v_store_id);
  END LOOP;
END;
...
BEGIN
  ...
  VALUES (auth.uid(), v_total, 'cash', v_store_id)
...
GRANT EXECUTE ON FUNCTION fulfill_shopify_order(JSONB, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION record_cod_payment(JSONB, UUID) TO authenticated;

-- Rattrapage : commandes déjà préparées avant cette migration
UPDATE shopify_orders
SET fulfilled_at = COALESCE(updated_at, created_at),
    fulfilled_by = paid_by
