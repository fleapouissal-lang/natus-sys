-- Gérant transferts : source = magasins associés (can_access_store), destination = tout retail / hub actif.
...
BEGIN
  ...
  IF NOT is_director_or_admin()
    AND NOT can_access_store(v_transfer.from_store_id)
    AND NOT can_access_store(v_transfer.to_store_id) THEN
    RAISE EXCEPTION 'Magasin hors périmètre';
  END IF;
