-- Dépôt : magasins retail associés (remplace hub_manager_assignments pour les transferts)
...
CREATE TABLE IF NOT EXISTS hub_store_assignments (
  ...
  UNIQUE (hub_user_id, store_id),
  UNIQUE (store_id)
);

CREATE INDEX IF NOT EXISTS idx_hub_store_assignments_hub
  ON hub_store_assignments (hub_user_id);

ALTER TABLE hub_store_assignments ENABLE ROW LEVEL SECURITY;

CREATE POLICY hub_store_assignments_director_all ON hub_store_assignments
  FOR ALL TO authenticated
  USING (is_director())
  WITH CHECK (is_director());

CREATE POLICY hub_store_assignments_hub_read ON hub_store_assignments
  FOR SELECT TO authenticated
  USING (hub_user_id = auth.uid() OR is_director());

-- Rattacher les dépôts existants à tous les magasins retail de leur ville
INSERT INTO hub_store_assignments (hub_user_id, store_id)
SELECT h.id, s.id
FROM profiles h
JOIN stores s
  ON s.city = h.city
  AND s.is_active = true
  AND COALESCE(s.is_hub, false) = false
WHERE h.role = 'hub'
