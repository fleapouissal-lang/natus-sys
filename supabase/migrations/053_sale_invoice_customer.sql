-- Facture : infos client sur chaque vente (Shopify, fidélité ou « Divers »)

ALTER TABLE sales
  ADD COLUMN IF NOT EXISTS customer_name TEXT NOT NULL DEFAULT 'Divers',
  ADD COLUMN IF NOT EXISTS customer_phone TEXT,
  ADD COLUMN IF NOT EXISTS customer_email TEXT,
  ADD COLUMN IF NOT EXISTS shopify_order_id UUID REFERENCES shopify_orders(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_sales_shopify_order ON sales(shopify_order_id);
...
WHERE s.customer_id = c.id
  AND s.customer_name = 'Divers';

UPDATE sales s
SET
  customer_name = COALESCE(NULLIF(trim(so.customer_name), ''), 'Divers'),
  customer_phone = so.customer_phone,
  customer_email = so.customer_email,
  shopify_order_id = so.id
FROM shopify_orders so
WHERE so.sale_id = s.id
  AND s.shopify_order_id IS NULL;
...
DROP FUNCTION IF EXISTS record_cod_payment(JSONB, UUID);
...
GRANT EXECUTE ON FUNCTION fulfill_shopify_order_sale(UUID, JSONB, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION record_cod_payment(JSONB, UUID, TEXT, TEXT, TEXT, UUID) TO authenticated;
