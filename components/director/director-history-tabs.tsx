...
function DirectorHistoryTabsInner({
  activities,
  activityScopeLabel,
  showStoreColumn,
  sales,
  ...
  const requestedTab: HistoryTab | null =
    tabParam === "activity" || tabParam === "sales" || tabParam === "closures"
      ? tabParam
      : null;
  const defaultTab: HistoryTab = showActivityTab
    ? "activity"
    : showSalesTab
      ? "sales"
      : "closures";
...
export function DirectorHistoryTabs(props: {
  activities: ActivityEntry[];
  activityScopeLabel: string;
  showStoreColumn: boolean;
  sales: Sale[];
  ...
  return (
    <Suspense fallback={null}>
      <DirectorHistoryTabsInner {...props} />
    </Suspense>
  );
}
