...
import type { LoyaltyCustomer, LoyaltySettings, LoyaltyStats } from "@/lib/types";

type ClientTab = "normal" | "pro" | "programme";

function parseClientTab(tabParam: string | null, initialTab: ClientTab): ClientTab {
  if (tabParam === "programme" || tabParam === "loyalty") return "programme";
  if (tabParam === "pro-plus") return "normal";
  if (tabParam === "pro") return "pro";
  if (initialTab === "pro" || initialTab === "programme") return initialTab;
  return "normal";
}
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted">
        {tab === "normal"
          ? `${normalClients.length} carte${normalClients.length !== 1 ? "s" : ""} fidélité standard — points uniquement, sans remise Pro${inactiveNormalCount > 0 ? ` · ${inactiveNormalCount} désactivée${inactiveNormalCount !== 1 ? "s" : ""}` : ""}`
          : tab === "pro"
            ? `${proClients.length} compte${proClients.length !== 1 ? "s" : ""} pro · ${pendingProCount} en attente d'activation`
            : tab === "programme"
              ? "Paramètres du programme et statistiques globales"
              : ""}
      </p>

      {tab === "programme" ? (
        <div className="space-y-6">
          <LoyaltySettingsForm initialSettings={loyaltySettings} />
          <LoyaltyDashboard
            stats={loyaltyStats}
            customerBasePath={detailBasePath}
            loyaltySettings={loyaltySettings}
          />
        </div>
      ) : (
