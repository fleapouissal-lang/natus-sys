...
export default async function DirectorShopifyOrdersPage({
  searchParams,
}: {
  ...
  const selectedCityForRoute = cityParam || null;
  ...
  const hubStore = selectedCityForRoute ? await getHubStore(selectedCityForRoute) : null;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted">
            Commandes Shopify affectées aux magasins — le hub stock a sa propre page
          </p>
        </div>
        <ShopifySyncButton />
      </div>
