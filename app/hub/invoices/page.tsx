...
export default async function HubInvoicesPage({
  ...
  const {
    ...
    showStore,
    showCashier,
  } = await loadInvoicesListPage("hub", params);

  return (
    <div className="animate-fade-in space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Factures magasins</h1>
        <p className="mt-1 text-muted">
          Factures des magasins retail affectés à votre hub
        </p>
      </div>
      ...
      {stores.length === 0 ? (
        <Card className="py-12 text-center text-muted">
          Aucun magasin retail dans votre ville
        </Card>
      ) : (
