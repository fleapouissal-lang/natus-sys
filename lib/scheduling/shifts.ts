...
import { getPlanningCashiersForStores } from "@/lib/scheduling/planning-cashiers";
...
export async function getCityCashiers(storeIds: string[]): Promise<CashierWithStore[]> {
  const rows = await getPlanningCashiersForStores(storeIds);
  return rows.map(({ sort_order: _sort, ...cashier }) => cashier);
}
...
export async function getCashierShifts(input: {
  ...
  return (data || []).map((row) => {
    ...
    return {
      ...shift,
      profiles: cashier
        ? { full_name: cashier.full_name, email: "" }
        : null,
      stores: store ?? null,
    };
  });
