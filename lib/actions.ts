...
export async function bulkImportLoyaltyCustomers(
  rows: Array<{
    line: number;
    fullName: string;
    phone: string;
  ...
  const profile = await requireRole(["directeur", "admin"]);
  if (!profile) return { created: 0, errors: ["Non autorisé"] };
  if (rows.length === 0) return { created: 0, errors: ["Aucune ligne à importer"] };
  if (rows.length > 500) {
    return { created: 0, errors: ["Maximum 500 lignes par import"] };
  }
  ...
  for (const row of rows) {
    ...
    if ("error" in result) {
      errors.push(`Ligne ${row.line} (${row.fullName}) : ${result.error}`);
      continue;
    }
    created++;
  }
...
export async function bulkImportProClients(
  rows: Array<{
    line: number;
  ...
  for (const row of rows) {
    const storeId = storeByName.get(row.storeName.trim().toLowerCase());
    if (!storeId) {
      errors.push(`Ligne ${row.line} : magasin « ${row.storeName} » introuvable`);
      continue;
    }
    ...
    const result = await createProClientCustomer({
      ...
      companyIce: row.companyIce,
      companyRc: row.companyRc,
    });

    if ("error" in result) {
      errors.push(`Ligne ${row.line} (${row.email}) : ${result.error}`);
      continue;
    }

    if (row.activate) {
      const activation = await toggleProClientActive(result.customerId, true);
      if ("error" in activation) {
        errors.push(
          `Ligne ${row.line} (${row.email}) : créé mais activation échouée — ${activation.error}`
        );
      }
    }
