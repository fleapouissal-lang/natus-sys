...
function drawHorizontalBars(
  ...
  labels.forEach((label, index) => {
    ...
    ctx.textAlign = "right";
    ctx.fillText(
      `${formatMoney(values[index])} (${percent.toFixed(0)}%)`,
      width - 24,
      y + 14
    );
...
function drawPieChart(
  ...
  labels.forEach((label, index) => {
    ...
    ctx.fillText(`${label} — ${formatMoney(values[index])} (${pct.toFixed(0)}%)`, 382, legendY + 12);
    legendY += 28;
  });
...
function drawHourlyChart(
  ...
  const filtered = values.map((v, i) => ({ v, l: labels[i] }));
  return drawBarChart(
    filtered.map((f) => f.l),
    filtered.map((f) => f.v),
    title,
...
export async function renderDashboardChartImages(
  ...
  if (analytics.dailySeries.some((d) => d.revenue > 0)) {
    charts.push({
      title: "Évolution du CA journalier",
      buffer: await drawBarChart(
        analytics.dailySeries.map((d) => d.label),
        analytics.dailySeries.map((d) => d.revenue),
        `CA journalier — ${analytics.periodLabel}`
      ),
      width: 920,
      height: 360,
    });
  ...
  if (analytics.paymentMix.length > 0) {
    charts.push({
      title: "Répartition des paiements",
      buffer: await drawPieChart(
        analytics.paymentMix.map((p) => p.label),
        analytics.paymentMix.map((p) => p.amount),
        `Paiements — ${analytics.periodLabel}`
      ),
      width: 640,
      height: 360,
    });
  ...
  if (analytics.hourlySeries.some((h) => h.sales > 0)) {
    charts.push({
      title: "Activité par heure",
      buffer: await drawHourlyChart(
        analytics.hourlySeries.filter((_, i) => i % 2 === 0).map((h) => h.label),
        analytics.hourlySeries.filter((_, i) => i % 2 === 0).map((h) => h.sales),
        `Ventes par heure — ${analytics.periodLabel}`
      ),
      width: 920,
      height: 360,
    });
  ...
  if (analytics.storeRanking.length > 1) {
    ...
    charts.push({
      ...
      buffer: await drawHorizontalBars(
        ...
        top.map((s) => s.revenue),
        top.map((s) => s.share),
        `CA par magasin — ${analytics.periodLabel}`
      ),
      width: 920,
  ...
  if (analytics.topProducts.length > 0) {
    ...
    charts.push({
      ...
      buffer: await drawHorizontalBars(
        ...
        `Top produits (CA) — ${analytics.periodLabel}`
      ),
      width: 920,
      height: Math.max(280, 80 + top.length * 36),
    });
