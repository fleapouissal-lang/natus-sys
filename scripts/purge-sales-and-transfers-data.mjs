/**
 * Purge ventes + commandes + transferts de stock.
 * Conserve : utilisateurs, clients, produits, catégories, magasins, stock actuel.
 *
 * Usage : node scripts/purge-sales-and-transfers-data.mjs
 */
import { createClient } from "@supabase/supabase-js";
import { loadEnv } from "./lib/env.mjs";

const TRANSFER_DELETE_ORDER = [
  "hub_stock_transfer_items",
  "store_stock_transfer_items",
];

const SALE_DELETE_ORDER = [
  "sale_cheques",
  "sale_items",
...
async function clearTable(supabase, table) {
  const strategies = [
    ...
    () => supabase.from(table).delete().neq("transfer_id", "00000000-0000-0000-0000-000000000000"),
  ];
...
async function purgeWithoutRpc(supabase) {
  ...
  const { error: transferMovementsError } = await supabase
    .from("stock_movements")
    .delete()
    .or("type.eq.transfer,hub_transfer_id.not.is.null,store_transfer_id.not.is.null");

  if (transferMovementsError) throw transferMovementsError;
  ...
  for (const table of SALE_DELETE_ORDER) {
    await clearTable(supabase, table);
  }
...
async function main() {
  ...
  console.log(
    "🗑️  Purge ventes + transferts — conserve utilisateurs, clients, produits, catégories, stock actuel\n"
  );

  const { data, error } = await supabase.rpc("purge_sales_and_transfers_data");
  ...
  if (error) {
    ...
    console.log("✓  purge_sales_and_transfers_data (RPC)", data);
  }
  ...
  const [
    ...
    { count: salesCount },
    { count: ordersCount },
    { count: storeTransfersCount },
    { count: hubTransfersCount },
  ] = await Promise.all([
  ...
  console.log(`↔️  Transferts magasin restants : ${storeTransfersCount ?? "?"}`);
  console.log(`↔️  Transferts hub restants : ${hubTransfersCount ?? "?"}`);
  console.log("\n✅ Purge terminée\n");
}
